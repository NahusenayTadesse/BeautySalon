<script>
  ...
  import { Button } from "$lib/components/ui/button/index.js";
	import { CalculatorIcon, CalendarIcon, CreditCardIcon, Disc, Search, SettingsIcon, SmileIcon, Trash, UserIcon } from "@lucide/svelte";
...
<Dialog.Root bind:open={isOpen}>
 <Dialog.Trigger class="w-auto px-4"
  ><Button variant="ghost" title="Search for Pages"><Search /> </Button></Dialog.Trigger
 >
 <Dialog.Content class="w-full">
  ...
  <Command.List>
    ...
    {#each list as list}
      <Command.Item>
        <Disc />
        <a href={list.path} onclick={()=> isOpen = false} >{list.label}</a>
      </Command.Item>
    {/each}
    </Command.Group>
  </Command.List>
